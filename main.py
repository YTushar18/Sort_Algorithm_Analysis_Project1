...
from sorting_algorithms import bubble_sort, selection_sort, insertion_sort, merge_sort, quick_sort, radix_sort, heap_sort  # Import your sorting algorithms
...
class SortingApp:
    def __init__(self, root):
        ...
        # UI Element for random array generation
        self.random_array_button = ttk.Button(self.input_frame, text="Generate", command=self.generate_random_array)
        self.array_label = ttk.Label(self.input_frame, text="Array Values (comma-separated):")
        self.array_entry = ttk.Entry(self.input_frame)
        self.length_label = ttk.Label(self.input_frame, text="Array Length:")
    ...
    def display_graph(self, algorithm_runtimes):
        ...
        # Extracting algorithm names and runtimes
        ...
        self.ax.set_title('Sorting Algorithm Runtimes')
         ...
         # Adjust x-axis label formatting only if there are more than 3 algorithms
        if len(algorithms) > 3:
            for label in self.ax.get_xticklabels():
                label.set_fontsize(6)  # Adjust the font size as needed
